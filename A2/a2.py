...
class Assignment2:
    ...
    def dispatch(self, nw: GeoLoc, se: GeoLoc, when: datetime) -> None:
        ...
        view_BusyDriver_query = ('CREATE VIEW BusyDriver AS ' + 
            'SELECT driver_id ' + 
            'FROM Dispatch, ClockedIn ' + 
            'WHERE Dispatch.shift_id = ClockedIn.shift_id AND ' + 
            'Dispatch.request_id NOT IN ' + 
            '(SELECT request_id FROM dropoff); ')


        view_NotBusyDriver_query = ('CREATE VIEW NotBusyDriver AS ' + 
            '(SELECT driver_id FROM Driver) ' + 
            ' EXCEPT ' + 
            '(SELECT driver_id FROM BusyDriver); ') 


        view_OnShiftDriver_query = ('CREATE VIEW OnShiftDriver AS ' + 
            'SELECT DISTINCT Driver.driver_id AS driver_id, ClockedIn.shift_id AS shift_id ' + 
            'FROM Driver, ClockedIn ' + 
            'WHERE Driver.driver_id = ClockedIn.driver_id AND ' + 
            'NOT EXISTS (SELECT * ' + 
            'FROM ClockedOut ' + 
            'WHERE ClockedOut.shift_id = ClockedIn.shift_id); ')


        view_MostRecentTime_query = ('CREATE VIEW MostRecentTime AS ' + 
            'SELECT driver_id, MAX(Location.datetime) AS recenttime ' + 
            'FROM ClockedIn, Location ' + 
            'WHERE ClockedIn.shift_id = Location.shift_id ' + 
            'GROUP BY driver_id; ') 

        view_MostRecentLocation_query = ('CREATE VIEW MostRecentLocation AS ' + 
            'SELECT ClockedIn.driver_id AS driver_id, Location.location AS location ' + 
            'FROM MostRecentTime, ClockedIn, Location ' + 
            'WHERE MostRecentTime.driver_id = ClockedIn.driver_id AND ' + 
            'ClockedIn.shift_id =  Location.shift_id AND ' + 
            'MostRecentTime.recenttime = Location.datetime; ') 

        
        view_NearDriver_query = ('CREATE VIEW NearDriver AS ' + 
            'SELECT driver_id, location ' + 
            'FROM MostRecentLocation ' + 
            'WHERE location[0] <= %s AND ' + 
            'location[0] >= %s AND ' + 
            'location[1] <= %s AND ' + 
            'location[1] >= %s ; ') 

            
        view_AvailableDriver_query = ('CREATE VIEW AvailableDriver AS ' + 
            'SELECT driver_id, shift_id, location AS car_location ' + 
            'FROM NearDriver NATURAL JOIN OnShiftDriver NATURAL JOIN NotBusyDriver ; ')

        
        view_RequestInNeed_query = ('CREATE VIEW RequestInNeed AS ' + 
            'SELECT client_id, request_id, source ' + 
            'FROM Request ' + 
            'WHERE Request.request_id NOT IN ' + 
            '(SELECT Dispatch.request_id FROM Dispatch) AND ' + 
            'source[0] <= %s AND ' +
            'source[0] >= %s AND ' + 
            'source[1] <= %s AND ' + 
            'source[1] >= %s ; ') 
        ...
        view_TotalBilled1_query = ('CREATE VIEW TotalBilled1 AS ' + 
            'SELECT client_id, SUM(amount) AS totalpaid  ' + 
            'FROM Request, Billed  ' + 
            'WHERE Request.request_id = Billed.request_id  ' + 
            'GROUP BY client_id    ; ') 

        view_TotalBilled2_query = ('CREATE VIEW TotalBilled2 AS ' + 
            'SELECT client_id, 0 AS totalpaid  ' + 
            'FROM Client  ' + 
            'WHERE Client.client_id NOT IN  ' + 
            '(SELECT client_id FROM TotalBilled1) ; ') 

        view_TotalBilled_query = ('CREATE VIEW TotalBilled AS ' + 
            '(SELECT * FROM TotalBilled1)  ' + 
            'UNION  ' + 
            '(SELECT * FROM TotalBilled2) ; ') 

        view_RequestByBilled_query = ('CREATE VIEW RequestByBilled AS ' + 
            'SELECT RequestInNeed.client_id AS client_id, request_id, source, totalpaid ' + 
            'FROM RequestInNeed, TotalBilled  ' + 
            'WHERE RequestInNeed.client_id = TotalBilled.client_id  ' + 
        ...
        try:
            ...
            cursor3.execute(view_BusyDriver_query)

            cursor3.execute(view_NotBusyDriver_query)

            cursor3.execute(view_OnShiftDriver_query)

            cursor3.execute(view_MostRecentTime_query)

            cursor3.execute(view_MostRecentLocation_query)

            cursor3.execute(view_NearDriver_query, [se.longitude, nw.longitude, nw.latitude, se.latitude])

            cursor3.execute(view_AvailableDriver_query)

            cursor3.execute(view_RequestInNeed_query, [se.longitude, nw.longitude, nw.latitude, se.latitude])

            cursor3.execute(view_TotalBilled1_query)

            cursor3.execute(view_TotalBilled2_query)

            cursor3.execute(view_TotalBilled_query)

            cursor3.execute(view_RequestByBilled_query)

            allRequest = []

            allDriver = []

            cursor3.execute("SELECT * FROM RequestByBilled;")
            for row in cursor3:
                allRequest.append(row)

            cursor3.execute("SELECT * FROM AvailableDriver;")
            for row in cursor3:
                allDriver.append(row)
            ...
            for r in allRequest:
                ...
                for i in range(len(allDriver)):
                    new_sqdis = pow((allDriver[i][2].longitude - r[2].longitude),2) + pow((allDriver[i][2].latitude - r[2].latitude),2)
                    if new_sqdis < sqdis:
                        sqdis = new_sqdis
                        ind = i
                
                cursor3.execute("INSERT INTO Dispatch VALUES (%s, %s, %s, %s);", [r[1], allDriver[i][1], allDriver[i][2], when])
